...
def tr_acc(model, image, image_LIDAR, label, diffusion):
    ...
    for idx, (image_batch, image_LIDAR_batch, label_batch) in enumerate(train_loader):
        trans_image_batch = image_batch.cuda(1)
        trans_image_LIDAR_batch = image_LIDAR_batch.cuda(1)
        label_batch = label_batch.cuda(1)
        t = torch.randint(0, diffusion.num_timesteps, (trans_image_batch.shape[0],)).cuda(1)
        logits = model(trans_image_batch, trans_image_LIDAR_batch, t)
...
def pre_train(model, train_image, train_image_LIDAR, train_label,validation_image, validation_image_LIDAR, validation_label, epoch, optimizer, scheduler, bs, diffusion, val = False, ):
    ...
    for i in range(epoch): 
        ...
        train_acc, tr_loss = tr_acc(model.eval(), train_image, train_image_LIDAR, train_label, diffusion)
        val_acc, val_loss= tr_acc(model.eval(), validation_image, validation_image_LIDAR, validation_label, diffusion)
        if val_acc > BestAcc:
...
def test_batch(model, image, image_LIDAR, index, BATCH_SIZE,  nTrain_perClass, nvalid_perClass, halfsize, diffusion):
    ...
    for i in range(n):
        ...
        t = torch.randint(0, diffusion.num_timesteps, (image_b.shape[0],)).cuda(1)
        logits = model(torch.tensor(image_b).cuda(1), torch.tensor(image_LIDAR_b).cuda(1), t)
        if isinstance(logits,tuple):
            logits = logits[-1]            
